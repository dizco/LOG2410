...
: MachineAbs(FiltrePtr(new Filtre)), m_CircuitEauChaude(new CircuitLiqComposite), m_CircuitThe(new CircuitSolComposite)
{
	...
	m_CircuitEauChaude->addSousElement( new Reservoir(4.5, 25.0, 20.0, 2200.0) );
	m_CircuitEauChaude->addSousElement(new Tuyau(0.25, 5.0));
	m_CircuitEauChaude->addSousElement(new Pompe(0.05f, 10.0));
	m_CircuitEauChaude->addSousElement(new Tuyau(0.25, 5.0));
	m_CircuitEauChaude->addSousElement(new Bouilloire(5.5, 20.0,500.0));
	m_CircuitEauChaude->addSousElement(new Tuyau(0.25, 5.0));

	// Constuire les elements du circuit de the
	m_CircuitThe->addSousElement(new Moteur(5.0));
...
void MachineBase::infuserThe(float duree){
	...
	// Constuire un programme pour commander la machine
	ProgrammeMachine progLiquide;
	progLiquide.addCommande(new CommandeTransfertLiqReservBouil(150));
	progLiquide.addCommande(new CommandeFixerTemperatureLiquide(90));

	// Recuperer l'executeur de commande et lui faire executer le programme
	const ExecuteurCommandes* executeur = ExecuteurCommandes::getInstance();

	executeur->executer(&progLiquide, m_CircuitEauChaude.get());
